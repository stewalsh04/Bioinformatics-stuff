...
bwa mem $TMP_DIR/$REFASTA $TMP_DIR/*fsp.fq.gz > $TMP_DIR/$EVX_RUNID.sam || pipeline_error  "Alignment Error"
...
# convert .sam to .bam
samtools view -bSq 2 $TMP_DIR/$EVX_RUNID.sam > $TMP_DIR/${EVX_RUNID}_x1.bam || pipeline_error "Samtools Bam convert Error"
# samtools: read Q30 reads only (above q30 quality) and pipe to a new .bam - essentially filtering on quality
samtools view -q 30 -b $TMP_DIR/${EVX_RUNID}_x1.bam >  $TMP_DIR/${EVX_RUNID}_x2.bam || pipeline_error "Samtools Filter Error"
...
if (( $postfilternumber < 100 )); then
    ...
    #Run the whats in it? script to see what has been sequenced
    python unaligned_reads.py $TMP_DIR/ $SYNDNA $EVX_RUNID
    
    #Carry out a multiple alignment on output from whats in it
    muscle -in $TMP_DIR/*_top_${UNALIGNEDTHRESH}_reads.fasta -out $TMP_DIR/${EVX_RUNID}_top_${UNALIGNEDTHRESH}_reads.aligned.fasta.tmp

    #Re-order the aligment so the most abundant variants are at the top (Using amended python script from Muscle author)
    python stable.py $TMP_DIR/*_top_${UNALIGNEDTHRESH}_reads.fasta $TMP_DIR/${EVX_RUNID}_top_${UNALIGNEDTHRESH}_reads.aligned.fasta.tmp > $TMP_DIR/${EVX_RUNID}_top_${UNALIGNEDTHRESH}_reads.aligned.fasta

    #Remove tempory alignment files
    rm $TMP_DIR/${EVX_RUNID}_top_${UNALIGNEDTHRESH}_reads.aligned.fasta.tmp

    #Make the multiple alignment plots using pymsaviz (https://pypi.org/project/pymsaviz/)
    python unaligned_reads_plot.py $TMP_DIR/${EVX_RUNID}_top_${UNALIGNEDTHRESH}_reads.aligned.fasta $EVX_RUNID $TMP_DIR
...
echo "*******************************************"
python ngs_python_wrapper.py $TMP_DIR/${EVX_RUNID}_var_${SEQ}.csv $TMP_DIR/${EVX_RUNID}_matches.csv $TMP_DIR/${EVX_RUNID}_insert_size.csv \
$TMP_DIR/${EVX_RUNID}.sam $SYNDNA $TMP_DIR $EVX_RUNID $TMP_DIR/$REFASTA ${THRESH%%.*} || pipeline_error "Error with python plotting scripts"
...
if [ $SUMMARY_STATS == "true" ]; then
    cp $TMP_DIR/${EVX_RUNID}_stats.csv $HOMEPATH/ngs_pipeline/outputs/$MULTIRUNID/summary-stats
fi
...
python unaligned_reads.py $TMP_DIR/ $SYNDNA $EVX_RUNID $UNALIGNEDTHRESH
